...
def save_assessment_frames(
    df: pd.DataFrame, out_dir: str = "artifacts/assessments", stem: str | None = None
) -> Dict[str, str]:
    out = Path(out_dir)
    ...
    )  # pandas to_csv :contentReference[oaicite:6]{index=6}
    ...
    if have_pyarrow or have_fastparquet:
        ...
        df.to_parquet(
            pq_path, index=False
        )  # requires pyarrow/fastparquet :contentReference[oaicite:7]{index=7}
